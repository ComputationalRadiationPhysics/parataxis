...
struct DataBoxReader<PMacc::math::Complex<T_ValueType>>
{
            ...
            const PMacc::DataSpace<T_globalDims>& localOffset)
    {
        ...
        readDataBox(writer["real"], makeHostTransformBox(dataBox, [](ValueType& value) -> T_ValueType& { return value.get_real(); }), globalDomain, localSize, localOffset);
        readDataBox(writer["imag"], makeHostTransformBox(dataBox, [](ValueType& value) -> T_ValueType& { return value.get_imag(); }), globalDomain, localSize, localOffset);
    }
};
